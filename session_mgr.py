...
class SessionManager:
    ...
    async def add_client(self, phone, client, me=None):
        if me is None:
            try:
                ...
                me = None
        self.clients[phone] = (client, me)
        logger.info(f"✅ Client ajouté: {phone}")
    ...
    async def remove_client(self, phone):
        if phone in self.clients:
            ...
            logger.info(f"🗑️ Client retiré: {phone}")
    ...
    async def load_all_active_accounts(self):
        ...
        logger.info(f"📊 {loaded}/{len(accounts)} comptes connectés")
        return loaded  # ← C'EST ÇA QUI MANQUAIT
